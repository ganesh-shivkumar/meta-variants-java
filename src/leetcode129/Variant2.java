...
public class Variant2 {
    ...
    public static class Entry{
        ...
        public Entry(TreeNode node, int value, boolean negative){
            this.node = node;
            this.value = value;
    ...
    public static int sumRootToLeaf(TreeNode root){
        ...
        boolean isRootNegative = (root.val < 0 ) ? true : false;
        queue.add(new Entry(root, 0, isRootNegative));

        while(!queue.isEmpty()){
            Entry current = queue.poll();
            TreeNode node = current.node;
            int value = current.value;
            boolean negative = (node.val<0) ? !current.negative : current.negative;

            String nodeValStr = String.valueOf(Math.abs(node.val));
            for(int i=0; i< nodeValStr.length(); i++){
                value = value * 10;
            }
            value = value + Math.abs(node.val);
            ...
            if(node.left!=null){
                queue.add(new Entry(node.left, value, negative));
            }
            if(node.right!=null){
                queue.add(new Entry(node.right, value, negative));
            }
        }
        return sum;
    }
    ...
    public static void main(String[] args) {
        ...
        int expected1 = -129 + -145; // Corrected expected value based on the logic
        ...
        int expected2 = 0 - 1293 + 1293 -145 -146; // Corrected expected value
        ...
        int expected4 = -12 + -13; // Corrected expected value
